...
from api.models import User, Post, Newsletter
...
class UserSchema(ma.SQLAlchemySchema):
    ...
    posts_url = ma.URLFor("posts.user_all", values={"id": "<id>"}, dump_only=True)
    ...
    def validate_username(self, value):
        ...
        if value != old_username and db.session.scalar(
            User.select().filter_by(username=value)
        ):
...
class PostSchema(ma.SQLAlchemySchema):
    ...
    id = ma.auto_field(dump_only=True)
    url = ma.String(dump_only=True)
    text = ma.auto_field(required=True, validate=validate.Length(min=1, max=280))
