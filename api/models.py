...
class Newsletter(Updateable, db.Model):
    ...
    confirm_token = sqla.Column(sqla.String(64), nullable=False, index=True)
    leave_token = sqla.Column(sqla.String(64), nullable=False, index=True)
...
class User(Updateable, db.Model):
    ...
    id = sqla.Column(sqla.Integer, primary_key=True)
    username = sqla.Column(sqla.String(64), index=True, unique=True, nullable=True)
    email = sqla.Column(sqla.String(120), index=True, unique=True, nullable=False)
    ...
    def followed_posts_select(self):
        return (
            ...
            .group_by(Post.id)
            .filter(sqla.or_(Post.author == self, followers.c.follower_id == self.id))
        )
...
class Post(Updateable, db.Model):
    ...
    timestamp = sqla.Column(
        sqla.DateTime, index=True, default=datetime.utcnow, nullable=False
    )
