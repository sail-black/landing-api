...
class AuthTests(BaseTestCase):
    ...
    def test_get_token(self):
        ...
        rv = self.client.get(
            "/api/users", headers={"Authorization": f'Bearer {access_token + "x"}'}
        )
        assert rv.status_code == 401

        rv = self.client.get(
            "/api/users", headers={"Authorization": f"Bearer {refresh_token}"}
        )
        assert rv.status_code == 401
    ...
    def test_refresh_token(self):
        ...
        rv = self.client.put(
            "/api/tokens",
            json={"access_token": access_token1},
        ...
        rv = self.client.get(
            "/api/users", headers={"Authorization": f"Bearer {access_token2}"}
        )
        assert rv.status_code == 200
        assert rv.json["data"][0]["username"] == "test"

        rv = self.client.get(
            "/api/users", headers={"Authorization": f"Bearer {access_token1}"}
        )
        assert rv.status_code == 401
    ...
    def test_refresh_token_failure(self):
        ...
        rv = self.client.put(
            "/api/tokens",
            json={"access_token": access_token, "refresh_token": refresh_token + "x",},
        )
        rv = self.client.put(
            "/api/tokens",
            json={"access_token": access_token + "x", "refresh_token": refresh_token,},
        )
        assert rv.status_code == 401
    ...
    def test_refresh_revoke_all(self):
        ...
        rv = self.client.put(
            "/api/tokens",
            json={"access_token": access_token2, "refresh_token": refresh_token2},
        )
        ...
        assert rv.status_code == 401
        rv = self.client.get(
            "/api/users", headers={"Authorization": f"Bearer {access_token3}"}
        )
        assert rv.status_code == 401

        rv = self.client.put(
            "/api/tokens",
            json={"access_token": access_token1, "refresh_token": refresh_token1},
        )
        assert rv.status_code == 401
        rv = self.client.put(
            "/api/tokens",
        ...
        rv = self.client.put(
            "/api/tokens",
            json={"access_token": access_token3, "refresh_token": refresh_token3},
        )
        assert rv.status_code == 401
    ...
    def test_revoke(self):
        ...
        rv = self.client.delete(
            "/api/tokens", headers={"Authorization": f"Bearer {access_token}"}
        )
    ...
    def test_reset_password(self):
        with mock.patch("api.tokens.send_email") as send_email:
            rv = self.client.post(
                "/api/tokens/reset", json={"email": "bad@example.com",}
            )
            assert rv.status_code == 204
            rv = self.client.post(
                "/api/tokens/reset", json={"email": "test@example.com",}
            )
            assert rv.status_code == 204
