...
class PostTests(BaseTestCase):
    def test_new_post(self):
        rv = self.client.post("/api/posts", json={"text": "This is a test post",})
    ...
    def test_edit_post(self):
        ...
        rv = self.client.put(
            f"/api/posts/{id}", json={"text": "This is a test post edited",}
        )
        assert rv.status_code == 200
    ...
    def test_post_feed(self):
        ...
        user2 = User(username="susan", email="susan@example.com", password="dog")
        db.session.add(user2)
        db.session.commit()
        user1.follow(user2)
        now = datetime.utcnow()
        post1 = Post(text="Post 1", author=user2, timestamp=now - timedelta(minutes=2))
        post2 = Post(text="Post 2", author=user1, timestamp=now - timedelta(minutes=1))
    ...
    def test_permissions(self):
        user = User(username="susan", email="susan@example.com", password="dog")
