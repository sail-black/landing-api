...
class UserTests(BaseTestCase):
    ...
    def test_create_invalid_user(self):
        rv = self.client.post(
            "/api/users",
            json={"username": "1user", "email": "user@example.com", "password": "dog"},
        )
        assert rv.status_code == 400
        rv = self.client.post(
            "/api/users",
            json={"username": "", "email": "user@example.com", "password": "dog"},
        )
        assert rv.status_code == 400
    ...
    def test_edit_me(self):
        rv = self.client.put("/api/me", json={"about_me": "I am testing",})
        assert rv.status_code == 200
        assert rv.json["username"] == "test"
        assert rv.json["email"] == "test@example.com"
    ...
    def test_edit_password(self):
        rv = self.client.put("/api/me", json={"password": "bar",})
        assert rv.status_code == 400
        rv = self.client.put(
            "/api/me", json={"old_password": "foo1", "password": "bar",}
        )
        assert rv.status_code == 400
        rv = self.client.put(
            "/api/me", json={"old_password": "foo", "password": "bar",}
        )
        assert rv.status_code == 200
        assert rv.json["username"] == "test"
        assert rv.json["email"] == "test@example.com"
