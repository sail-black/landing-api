...
class UserModelTests(BaseTestCase):
    def test_newsletter_hashing(self):
        u = User(username="tim", email="tim@example.com", newsletter=True)
        db.session.add(u)
        db.session.commit()
        assert u.newsletter
    ...
    def test_follow(self):
        ...
        for _ in range(2):
            ...
            assert (
                db.session.scalar(
                    sqla.select(sqla.func.count()).select_from(u1.following_select())
                )
                == 1
            )
            ...
            assert (
                db.session.scalar(
                    sqla.select(sqla.func.count()).select_from(u2.followers_select())
                )
                == 1
            )
    ...
    def test_follow_posts(self):
        ...
        now = datetime.utcnow()
        p1 = Post(
            text="post from john", author=u1, timestamp=now + timedelta(seconds=1)
        )
        p2 = Post(
            text="post from susan", author=u2, timestamp=now + timedelta(seconds=4)
        )
        p3 = Post(
            text="post from mary", author=u3, timestamp=now + timedelta(seconds=3)
        )
        p4 = Post(
            text="post from david", author=u4, timestamp=now + timedelta(seconds=2)
        )
        ...
        f1 = db.session.scalars(
            u1.followed_posts_select().order_by(Post.timestamp.desc())
        ).all()
        f2 = db.session.scalars(
            u2.followed_posts_select().order_by(Post.timestamp.desc())
        ).all()
        f3 = db.session.scalars(
            u3.followed_posts_select().order_by(Post.timestamp.desc())
        ).all()
        f4 = db.session.scalars(
            u4.followed_posts_select().order_by(Post.timestamp.desc())
        ).all()
        assert f1 == [p2, p4, p1]
        assert f2 == [p2, p3]
        assert f3 == [p3, p4]
